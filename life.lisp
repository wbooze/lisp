...
(defun gen-rand-universe (height width)
    (if (eq height 0)
	nil
	(cons (gen-rand-row width) (gen-rand-universe (1- height) width))))

(defun gen-rand-row (width)
  (if (eq width 0)
      nil
    (cons (random 2) (gen-rand-row (1- width)))))
	  
(defun construct-neighbors (universe row col)
  (let ((neighbors '()))
    (dotimes (i 3)
      (let ((new-row 
	     (wrapping-subseq (wrapping-nth (1- (+ row i)) universe) 
			      (1- col) (+ 2 col))))
  ...
  (if (eq start end)
      ...
      (let ((element
	     (cond ((< start 0) (nth (+ start (length seq)) seq))
		   ...
		   (t (nth start seq)))))
	(cons element (wrapping-subseq seq (1+ start) end)))))
