...
(defun foof (&optional (stream *standard-output*))
...
(defun bar (&optional (stream *standard-output*))
  (with-drawing-options (stream :ink (make-random-color))
    (formatting-item-list (stream)
      (formatting-cell (stream)
	(display-formula stream
...
(defun baz (&optional (stream *standard-output*))
  (with-drawing-options (stream :ink (make-random-color))
    (formatting-item-list (stream)
      (formatting-cell (stream)
	(display-formula stream
...
(defun foo (form &optional (stream *standard-output*))
  (with-drawing-options (stream :ink (make-random-color))
    (terpri)
    (formatting-item-list (stream)
       (formatting-cell (stream)
            (display-formula stream form))))
  (values))
