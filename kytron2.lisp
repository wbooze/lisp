(defpackage :kytron (:use :common-lisp))
(in-package "KYTRON") 
;;                            rm - february 1993
...
;;                            93/03/17 rm in 1.1 CLIM/MCL: 
;;                                     work-around for read-gesture-bug in run-simulation
;;                            93/08/23 rm in 1.1 CLIM: 
;;                                     bug fixed which caused loss of coord changes of moving obstacles
;;                            93/10/04 rm in 1.1 CLIM: 
;;                                     delete commands simplified 
;;                                     updating-output keyword bug fixed
;;                            93/11/25 rm in 2.0b CLIM: 
;;                                     started to upgrade to CLIM 2
;;                                       (filename: kytron2.lisp)
;;                                     [CLIM 1.x no longer supported]
...
(defvar *number-of-mounts* #+:mcl 24 #-:mcl 24) ;80
  ...
  (let ((cmds (append (compute-search-light-drive-commands k) (timer-commands k))))
    (cond ((and (member :left cmds) (member :right cmds))
          ...
          ((member :right cmds)
           ...
           (setf (speed k) (default-speed k)))
          (t (setf (speed k) 0))))
  (turn-left k (degree-to-rad (- (random 2.0) 1.0)))
  (step-forward k))
...
(defmethod drive-reversing ((k kytron3))
  (cond ((and (output-level (timer-rev-left k))
         ...
         ;;straight backwards
         (setf (speed k) (- (default-speed k)))
         (turn-left k (degree-to-rad (- (random 10.0) 5.0)))
         (step-forward k))
        ((output-level (timer-rev-left k))
         ;; backwards turn left
         (setf (speed k) (- (default-speed k)))
...
(defclass circle (geometric-object)
  ...
  (:default-initargs :radius 0 :centre (make-coord 0 0)))
...
(clim:define-application-frame kytron-on-the-moon ()
   ...
   (:panes (simulation :application 
            ...
            (documentation :pointer-documentation :text-style 
                   (clim:make-text-style :sans-serif :italic :very-small)
	         :max-height 12)
            (menu :command-menu
                   :text-style '(:sans-serif :bold :small)))
   (:layouts (default
	        (clim:vertically ()
		    simulation
		    documentation
...
;; define the iconic-view (was predefined in CLIM 1)
...
(clim:define-presentation-method clim:present 
  (k (type kytron) stream
     view &key)
  (declare (ignore view))
  (format stream "~s: ~s" k (slot-value k 'name)))
  ...
  (clim:updating-output 
   ...
   (map nil #'(lambda (x) 
                (setf (output-record x)
                      (clim:updating-output 
                       (stream :unique-id x
                               :cache-value (cons (x (location x)) (x (location x)))
                               :cache-test #'equal)
...
(defmethod draw-obstacle ((object light-source) stream)
  ...
  ;; this makes it slow but informative (you see the visibility border):
  (clim:draw-circle* stream (x (centre object)) (y (centre object))
...
(defun translate-color (color-name)
  "translate logical colornames into actual CLIM colors"
  ;; adapted for LispWorks by David L. Westbrook (westy@cs.umass.edu)
  (case color-name
    (cyan CLIM:+CYAN+)
    ...
    (red CLIM:+RED+)
    (green CLIM:+GREEN+)
    (orange      #-Lispworks CLIM:+ORANGE+
       #+Lispworks #.(clim::make-rgb-color  1 .7 0))
    (dark-orange #-Lispworks CLIM:+DARK-ORANGE+
       #+Lispworks #.(clim::make-rgb-color .8 .5 0))
    (black CLIM:+BLACK+)
    (white CLIM:+WHITE+)
    (dark-gray   #-Lispworks CLIM:+DIM-GRAY+
       #+Lispworks #.(clim::make-rgb-color .7 .7 .7))
    (middle-gray #-Lispworks CLIM:+GRAY+
       #+Lispworks #.(clim::make-rgb-color .5 .5 .5))
    (light-gray  #-Lispworks CLIM:+LIGHT-GRAY+
       #+Lispworks #.(clim::make-rgb-color .3 .3 .3))
    (gray        #-Lispworks CLIM:+GRAY+
       #+Lispworks #.(clim::make-rgb-color .5 .5 .5))
...
#+:clim-2
(clim:define-gesture-name :select :pointer-button (:left))
...
#+:mcl 
(clim:define-gesture-name :menu :button (:left :shift :control))
...
;; now with dragging by means of an implicit translator
(define-kytron-on-the-moon-command (com-move :menu t :name "Move Object") 
                                   ((m 'movable :gesture :select))
  (let ((stream (clim:get-frame-pane clim:*application-frame* 'simulation)))
    (multiple-value-bind (x y)
      ...
      (setf (location m) (add-coords (scale (direction m) (speed m))
                                 (make-coord x y))))))
      ;;)))
...
(define-kytron-on-the-moon-command (com-run-simulation :menu t) ()
  ...
  (block loops
    #+:genera (silica:inhibit-updating-scroll-bars #+:franz (clim:*application-frame*)
...
(define-kytron-on-the-moon-command (com-time-100-steps :menu t) ()
  ;; a benchmarking command
  (time
   #+:genera (silica:inhibit-updating-scroll-bars #+:franz (clim:*application-frame*)
   (dotimes (i 100) ;; 
     ;; One simu step for all KYTRONs:
...
;; CLIM 1 compatibility function (should be cleaned up)
(defun map-over-output-record-elements (record fun)
  (mapcar fun (clim:output-record-children record)))
