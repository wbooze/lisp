;;; -*- Syntax: Common-lisp; Package: CLIM -*-

;;; Hacks for efficiently drawing really big scientific graphs.
;;; This was written during part of a CLIM class with the help
;;; of Scott McKay.  This code runs in clim 1.1, but it has since
;;; been ported to clim 2 and is provided as one of its demos.
;;;
;;; The basic issue is that a graph of 100,000 data points could
;;; potentially consist of more than 100,000 presentations.  
;;; Using the standard behavior, clim can spend 80-90% of its
;;; time consing up presentations, only to throw them away upon
;;; the next redisplay.  This file shows how you can cons them
;;; up once and keep them around forever as part of the graph.
;;;
;;; Jeff Morrill
;;; jmorrill@bbn.com
...
[Here is the original message that accompanied this file.
 Vincent Keunen]

To: Scott McKay <SWM@stony-brook.scrc.symbolics.com>
cc: clim@BBN.COM, abbott@aero.org, keunen@nrb
Subject: Re: Clim Speed 
In-Reply-To: Scott McKay's message of Wed, 17 Feb 93 16:16:00 -0500.
             <19930217211640.8.SWM@SUMMER.SCRC.Symbolics.COM> 
Reply-To: jmorrill@BBN.COM
Date: Thu, 18 Feb 1993 00:18+0200
From: Jeff Morrill <jmorrill@BBN.COM>

  Date: Wed, 17 Feb 1993 16:16-0500
  From: Scott McKay <SWM@stony-brook.scrc.symbolics.com>
  Subject: Re: Clim Speed

  During a CLIM class I taught, Jeff Morrill
  wrote some custom output record code, which is part of the CLIM 2.0 demo
  system.  Perhaps he could be persuaded to forward the older CLIM 1.1
  code to you.

Here it is.  Perhaps Vincent would be kind enough to add it
to the clim library.  It should be called custom-records.lisp,
to be consistent with the clim demos.
...
;;; very similar to linear-output-record.  It is faster than linear-output-record
;;; for mouse-tracking because the inferiors are sorted and therefore
;;; binary search can be used to eliminate some of the unneeded mapping.

(defclass dataset-output-record
    (output-record-mixin output-record-element-mixin dataset)
    ())
...
(defmethod display-data ((dataset dataset-output-record) stream
			 &key (width 500) (height 60))
  (set-size dataset width height)
  (multiple-value-bind (x y) (stream-cursor-position* stream)
    (set-xy-position dataset x y))
  (let ((record
